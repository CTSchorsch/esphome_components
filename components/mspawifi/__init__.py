...
CONFIG_SCHEMA = cv.COMPONENT_SCHEMA.extend({
    ...
    cv.Optional(CONF_HEATER_SWITCH): switch.switch_schema({cv.GenerateID(): cv.declare_id(MSPAWifiHeaterSwitch)}),
    cv.Optional(CONF_FILTER_SWITCH): switch.switch_schema({cv.GenerateID(): cv.declare_id(MSPAWifiFilterSwitch)}),
    cv.Optional(CONF_SOLLTEMP_NUMBER): number.switch_schema({cv.GenerateID(): cv.declare_id(MSPAWifiSollTempNumber)}),
})
